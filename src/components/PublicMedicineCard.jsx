...
function PublicMedicineCard({ medicine }) {
  ...
  return (
    <>
      {/* Medicine Card */}
      <div 
        className={`relative rounded-2xl overflow-hidden cursor-pointer transition-all duration-400 ease-[cubic-bezier(0.25,0.46,0.45,0.94)] w-full max-w-xs mx-auto mb-4 min-h-[480px] flex flex-col
          ${isDarkMode ? 
            'bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-600/60' : 
            'bg-gradient-to-br from-white to-slate-50 border border-slate-200/60'
          }
      ...
      >
        ...
        <div className="p-6 flex flex-col flex-grow">
          <div className="flex-grow">
            ...
            <p className={`text-sm leading-relaxed mb-4 font-normal line-clamp-3 min-h-[3.5rem] 
              ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
              {medicine.description}
            </p>
          ...
          <Button 
            ...
            disabled={medicine.stock <= 0}
            onClick={handleAddToCart}
            className={`w-full h-12 rounded-2xl text-sm font-bold border-none transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] tracking-wide text-white mt-auto
              ${medicine.stock > 0 ? 
                'bg-gradient-to-br from-blue-500 to-blue-700  py-1.5 shadow-[0_4px_20px_rgba(59,130,246,0.4),inset_0_1px_0_rgba(255,255,255,0.2)]' : 
                'bg-gradient-to-br from-gray-500 to-gray-600 shadow-[0_4px_20px_rgba(107,114,128,0.3)]'
              }
